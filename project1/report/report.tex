...
\begin{algorithm}[H]
    ...
    \begin{algorithmic}
        ...
        \For{$i = 1, 2, \dots, n-1$}                \Comment $n-1$ iterations
            ...
            \State $g'_{i+1} = g_{i+1} - d\, g'_i$  \Comment 2 FLOPs 
        \EndFor
...
\begin{algorithm}[H]
    ...
    \begin{algorithmic}
        ...
        \State $g'_1 = g_1$
        \For{$i = 1, 2, \dots, n-1$}                        \Comment $(n-1)$ iterations
            \State $b'_{i+1} = 2 - 1/b'_i$                  \Comment 2 FLOPs 
            \State $g'_{i+1} = g_{i+1} + g'_{i}/b'_{i}$     \Comment 2 FLOPs 
        \EndFor
        \Comment Backward substitution
        \State $v_n = g'_n/b'_n$                            \Comment 1 FLOP 
        \For{$i = 1, 2, \dots, n-1$}                        \Comment $(n-1)$ iterations
            \State $v_{n-i} = (g'_{n-i} + v_{n-i+1})/b'_{n-i}$  \Comment 2 FLOPs
        \EndFor
    \end{algorithmic}
\end{algorithm}
