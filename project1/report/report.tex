...
\begin{equation}
    ...
    = h^2 f_i.
\end{equation} 
For the first and last equation we can use the boundary conditions and they then simplify to  
\begin{equation}
    \begin{split}
        \begin{pmatrix}
            2 & -1
...
\section*{Problem 5}
The vector $\bv{v}$, as defined above, includes only the subset of $v$ which is unknown. The complete solution, $\bv{v}^*$ with length $m$, also includes the two endpoints, $v_0$ and $v_N$, which are determined by the boundary conditions. That means
\begin{equation}
    m = n + 2 = N + 1.
\end{equation}
